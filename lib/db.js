...
export async function closeDbPool() {
  ...
  try {
    ...
    log("db_error", `Failed to close PG pool: ${err.message}`);
    return false;
  }
}
...
async function ensureWorkerRuntimeTables() {
  ...
  workerRuntimeTablesReady = (async () => {
    ...
    await query(`
      CREATE INDEX IF NOT EXISTS idx_worker_activity_events_scope_created
      ON worker_activity_events (tenant_id, wallet_id, created_at DESC)
    `);
...
export async function claimNextWorkerControlRequest({
  ...
  const params = [tenant_id, wallet_id];
  const commandFilter = [];

  if (Array.isArray(commands) && commands.length > 0) {
    const placeholders = commands.map((command) => {
      params.push(command);
      return `$${params.length}`;
    });
    commandFilter.push(`AND command IN (${placeholders.join(", ")})`);
  }
...
export async function claimWorkerControlRequestById({
  id,
  worker_id,
}) {
  const ready = await ensureWorkerRuntimeTables();
...
export async function requeueWorkerControlRequest({
  id,
  worker_id = null,
  payload = {},
  error = null,
  ...
  const res = await query(
    `UPDATE worker_control_requests
     ...
     WHERE id = $1
       AND ($2::text IS NULL OR worker_id = $2)
     RETURNING *`,
    [id, worker_id, JSON.stringify(payload || {}), error]
  );
...
export async function recordWorkerActivity({
  ...
  const res = await query(
    `INSERT INTO worker_activity_events (
      tenant_id, wallet_id, worker_id, level, event_type, message, payload, created_at
    )
...
export async function listWorkerActivityEvents({
  tenant_id = null,
  wallet_id = null,
  limit = 20,
} = {}) {
